...
function plan_link($row) {
	...
	$result = db_query("SELECT approval_date FROM ".TB_PREF."purch_plan WHERE sorder_no = ".db_escape($row['order_no']), "Error");
	$prow = db_fetch($result);
	$plan_app_date = $prow[0];
		$order = get_sales_order_header($row['order_no'], ST_SALESORDER);
		$order_app_date = $order['approval_date'];
		if(($order_app_date != NULL) && ($plan_app_date == NULL))
			return pager_link( _('Purchase Planning'), '/planning/purchase_order_plan.php?OrderNumber='.$row['order_no'], ICON_PLAN);
		else
			return '<i class="'.ICON_PLAN.'" style="color:grey;" disabled title="This order has not been approved yet or plan has already been approved"></i>';
}
...
function approve_link($row) {
	...
	$prow = db_fetch($result);
	$plan_app_date = $prow[0];
	if($plan_app_date == NULL)
		return '<a target="_blank" href="../../planning/manage/approve_so_plan.php?trans_no='.$row["order_no"].'&trans_type=30" onclick="javascript:openWindow(this.href,this.target); return false;" title="Approve"><i class="'.ICON_SUBMIT.'"></i></a>';
	else
		return '<i class="'.ICON_SUBMIT.'" style="color:grey;" disabled title="This order has already been approved"></i>';
}
...
// 			return pager_link( _('Purchase Order'), '/purchasing/po_entry_items.php?NewOrder=Yes &OrderNumber='.$row['order_no'], ICON_CART);
// 		}
// 		else
// 			return '<i class="'.ICON_CART.'" style="color:grey;" disabled title="Plan of this order has not been approved yet"></i>';
// }
...
//		_('Currency') => array('align'=>'center'),
	...
	array_append($cols, array(
		...
		array('fun'=>'approve_link', 'align'=>'center', 'ord' =>''),
//		array('fun'=>'order_link', 'align'=>'center', 'ord' =>''),
