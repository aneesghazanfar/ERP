...
if (isset($_GET['OrderNumber']) && is_numeric($_GET['OrderNumber'])) {
  ...
  $form_data['stock_id'] = $myrow['stk_code'];
	...
	$form_data['units'] = get_unit($myrow['stock_id']);
...
function edit(&$order, $order_no, $line , $maincat_id) {
	...
	$plan_id = find_row('Edit');
	if ($plan_id == $line && $line != -1) {
		...
		if ($item_info) {
			...
			stock_style_list_cells('style_id', $_POST['style_id'],  false,$order_no);
			$t_style_qty = get_order_qty($order_no, $_POST['style_id']);
			hidden('t_style_qty',$t_style_qty);
			qty_cell($t_style_qty);
			sales_items_list_cells(null,'stock_id',$value['stock_id'],'',false, $maincat_id);
			label_cell($unit);
			small_qty_cells(null, 'perpc', null, null, null, $dec);
			small_qty_cells(null, 'stk_extra', null, null, null, $dec);
...
start_table(TABLESTYLE, "width='80%'");
	$th = array(_('Style Id'), _('Total Qty'), _('Accessory Code'), _('Accessory Desc'), _('UoM'), _('Qty/Pc'), _('Ex Qty %'), _('Req Qty'), _('Req by'), _('Image'), '', '');
	table_header($th);
			start_row();
			global $SysPrefs;
			$editable_items = true;
			$plan_id = find_row('Edit');
			if ($plan_id == -1 && $editable_items)
				edit($_SESSION['form_data'], $order_no, -1, $maincat_id);

			foreach ($_SESSION['form_data'] as $key => $value) {
				start_row();
				if($plan_id != $key || !$editable_items){
...
end_table(1);
submit_center_first('addplan',_('Submit'),  _('Check entered data and save document'), 'default');
div_end();
end_form();
