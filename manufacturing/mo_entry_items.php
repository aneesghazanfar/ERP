...
if (isset($_GET['ModifyOrderNumber']) && is_numeric($_GET['ModifyOrderNumber'])) {
	...
	$_SESSION['page_title'] = _($help_context = 'Modify Manufacture Order #') . $_GET['ModifyOrderNumber'];
	create_new_po(ST_PURCHORDER, $_GET['ModifyOrderNumber']);
	copy_from_cart();
}
elseif (isset($_GET['NewOrder'])) {
	//Start - MUZZAMMIL - Set page tilte to Manufacturing Order Entry
	$_SESSION['page_title'] = _($help_context = 'Manufacturing Order Entry');
	//End - Set page tilte to Manufacturing Order Entry
...
elseif (isset($_GET['NewInvoice'])) {
	...
	if (isset($_GET['FixedAsset'])) {
		$_SESSION['page_title'] = _($help_context = 'Fixed Asset Manufacture Invoice Entry');
		$_SESSION['PO']->fixed_asset = true;
	}
	else
		$_SESSION['page_title'] = _($help_context = 'Direct Manufacture Invoice Entry');
}
...
if (isset($_GET['AddedID'])) {
	...
	if (!isset($_GET['Updated']))
		display_notification_centered(_('Manufacture Order has been entered'));
	else
		display_notification_centered(_('Manufacture Order has been updated') . " #$order_no");
	display_note(get_trans_view_str($trans_type, $order_no, _('&View this order')), 0, 1);

	display_note(print_document_link($order_no, _('&Print This Order'), $trans_type), 0, 1);

	display_note(print_document_link($order_no, _('&Email This Order'), $trans_type, false, 'printlink', '', 1));

	hyperlink_params($path_to_root . '/purchasing/po_receive_items.php', _('&Receive Items on this Manufacture Order'), 'PONumber='.$order_no);

	// TODO, for fixed asset
	hyperlink_params($_SERVER['PHP_SELF'], _('Enter &Another Manufacture Order'), 'NewOrder=yes');
	
	hyperlink_no_params($path_to_root.'/purchasing/inquiry/po_search.php', _('Select An &Outstanding Manufacture Order'));
...
elseif (isset($_GET['AddedGRN'])) {
	...
	hyperlink_params($path_to_root.'/purchasing/supplier_invoice.php', _('Entry manufacture &invoice for this receival'), 'New=1');
...
elseif (isset($_GET['AddedPI'])) {
	...
	display_notification_centered(_('Direct Manufacture Invoice has been entered'));
...
function handle_cancel_po() {
	...
	display_notification(_('This Manufacture order has been cancelled.'));

	hyperlink_params($path_to_root . '/purchasing/po_entry_items.php', _('Enter a new Manufacture order'), 'NewOrder=Yes');
	echo '<br>';
		...
		if ($allow_update == true) {
			...
			if ($allow_update) {
				$_SESSION['PO']->add_to_order (count($_SESSION['PO']->line_items), $_POST['stock_id'], input_num('qty'), 
					...
					//Start - MUZZAMIL - 28-Mar-2023 - added stk_code and stk_description
					$_SESSION['PO']->trans_type == ST_PURCHORDER ? $_POST['req_del_date'] : '', 0, 0, 0, 0, $_POST['stk_code'], get_description($_POST['stk_code']));
					//End - MUZZAMIL - 28-Mar-2023 - added stk_code and stk_description

				unset_form_variables();
				$_POST['stock_id']	= '';
			} 
			else 
				 display_error(_('The selected item does not exist or it is a kit part and therefore cannot be Manufactured.'));
...
display_mo_header($_SESSION['PO']);
echo '<br>';
//Start - MUZZAMMIL - 29-Mar-2023
//Adding if svc code updated then display added po items related to the maincat_id of that svc.
if ((list_updated('stock_id') && get_post('sorder_no') != 0) || (list_updated('sorder_no') && get_post('stock_id') != 0)) {
	...
	//Getting required fields from purch_plan_summary table
	$result = db_query("SELECT stk_code, plan_req, req_date FROM 0_purch_plan_summary WHERE maincat_id = ".db_escape($maincat_id)." AND sorder_no = ".db_escape(get_post('sorder_no'))." AND po_date = '0000-00-00'", "Error in getting required fields from purch_plan_summary table");
	//Adding items to po using pre-defined add_to_order() function.
	$i = count($_SESSION['PO']->line_items);
	while($row = db_fetch($result)){
		//Added stk_code and stk_desc to add_to_order() function
		$_SESSION['PO']->add_to_order($i, get_post('stock_id'), ceil($row['plan_req']), 0, get_purchase_price($_SESSION['PO']->supplier_id, get_post('stock_id')), 0, date('m/d/Y', strtotime($row['req_date'])), 0, 0, get_post('sorder_no'), get_post('maincat_id'), $row['stk_code'], get_description($row['stk_code']));
		$i++;
	}
}
//End - MUZZAMMIL - 29-Mar-2023
	
display_mo_items($_SESSION['PO']);
